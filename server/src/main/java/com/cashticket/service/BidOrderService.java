...
public class BidOrderService {
    ...
    public BidOrderResponse createOrder(BidOrderRequest req, Long userId) {

        // 1) 입찰할 자격·금액 검증 ― 기존 AuctionService 로직 재사용
        auctionService.validateBid(req.concertId(), userId, req.bidAmount());
        ...
        paymentRepo.save(new Payment(
                orderId, userId, req.concertId(),
                req.bidAmount(), PaymentStatus.PENDING
        ));

        // 4) 프론트로 반환
        return new BidOrderResponse(orderId, req.bidAmount());
    }
}
